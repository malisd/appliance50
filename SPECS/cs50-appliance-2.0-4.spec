...
/bin/cp -a /etc/pam.d/gdm-password /etc/pam.d/gdm.rpmsave
...
if [ -e /etc/httpd/conf.d/mod_suphp.conf ]; then
    /bin/cp -a /etc/httpd/conf.d/mod_suphp.conf /etc/httpd/conf.d/mod_suphp.conf.rpmsave
fi
...
/usr/bin/mysqladmin -s --user=jharvard password "crimson"
...
sed -i -e "/^\$cfg\['Servers'\]\[\$i\]\['hide_db'\]       = '';/s/''/'information_schema|mysql|performance_schema'/" /etc/phpMyAdmin/config.inc.php
